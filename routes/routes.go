...
func Setup(app *fiber.App, clientset *kubernetes.Clientset) {
	...
	// Per tenant
	v1.Get(":tenant/pods", controllers.GetTenantPods)
	v1.Get(":tenant/namespaces", controllers.GetTenantNamespaces)
	v1.Get(":tenant/serviceAccounts", controllers.GetTenantServiceAccounts)

	requests = v1.Group(":tenant/requests")
	requests.Get("/cpu", controllers.GetTenantCPURequestsSum)
	requests.Get("/memory", controllers.GetTenantMemoryRequestsSum)
	requests.Get("/storage", controllers.GetTenantStorageRequestsSum)
	requests.Get("/ingress", controllers.GetTenantIngressRequestsSum)

	costs = v1.Group(":tenant/costs")
	costs.Get("/cpu", controllers.GetTenantCPUCostSum)
	costs.Get("/memory", controllers.GetTenantMemoryCostSum)
	costs.Get("/storage", controllers.GetTenantStorageCostSum)
	costs.Get("/ingress", controllers.GetTenantIngressCostSum)
}
